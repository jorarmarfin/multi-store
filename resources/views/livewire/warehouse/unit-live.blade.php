<div>
    <form class="grid grid-cols-4 gap-3" wire:submit="saveUnit">
        <div class="form-group">
            ...
            <input type="text" class="form-text" id="code"
                   wire:model="form.code" placeholder="Nombre de la unida">
            @error('form.code')
        ...
        <table>
            ...
            @foreach ($units as $unit)
                <tr>
                    ...
                    <td>{{ $unit->active_text }}</td>
                    <td>
                        <button wire:click="editUnit({{ $unit->id }})" class="btn-secondary">
                            Editar
                        </button>
                        <button @click="$dispatch('alert-delete',{{$unit->id}})" class="btn-danger">
                            Eliminar
                        </button>
                    </td>
                </tr>
    ...
    $wire.on('alert-delete', (category_id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $wire.deleteUnit(category_id);
                Swal.fire(
                    'Eliminado!',
                    'La unida ha sido eliminada.',
                    'success'
                )
            }
        })
